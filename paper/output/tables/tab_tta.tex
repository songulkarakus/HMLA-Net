...
\bottomrule
\end{tabularx}
\noindent{\footnotesize{One checkpoint of HMLA-Net (replication B, seed 1234) evaluated under three inference groups, with latency at a batch size of one on the RTX A4000. The orbit dev.\ column is the largest absolute change of the orbit-averaged posterior when the input is transformed by an element of $G$ (floating-point noise only, i.e.\ exact invariance), and the raw dev.\ column is the same quantity for the single-view network, which is only approximately equivariant after training-time flip augmentation. The differences between the three rows (+0.53 and +1.17~pp of external macro-F1 for $V_4$ and $D_4$ against no TTA) are single-run measurements and lie within the replicate-to-replicate variation of Section~\ref{sec:res-replication}. The latency ratios are measured values for this system and batch size.}}
\end{table}
