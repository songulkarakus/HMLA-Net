...
\bottomrule
\end{tabularx}
\noindent{\footnotesize{Counts are after content-hash (MD5) de-duplication, which removed 76 exact duplicates from the 7{,}255 raw files. The imbalance ratio is 2.39:1. Every class is a folder of the form \texttt{<crop> leaf/<crop> <condition>}; the crop prefix is what the auxiliary crop head of Section~\ref{sec:arch} predicts. Class names are quoted as the corpus defines them (it writes \emph{jackfruit sooty mold}); the text uses \emph{mould}.}}
\end{table}
