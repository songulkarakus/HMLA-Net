...
| 3 Internal benchmark | A | `make_tables_rev.t_internal` (submitted body, unchanged); regenerated from the source by `make_tables_submitted.t_internal` and checked identical | `scripts/tables_submitted/tab_internal.tex`; source `replication_A/summary/summary_internal_test.csv`, caption numbers `submitted_version/numbers.json` |
...
| 7 Ablation | A (note: B) | `make_tables_rev.t_ablation` (submitted body; caption and note regenerated); body regenerated from the source by `make_tables_submitted.t_ablation` and checked identical | `scripts/tables_submitted/tab_ablation.tex`; source `submitted_version/_ablation_both.csv` (from `replication_A/runs/ablation_summary.csv` and the external metrics); B comparison `N.ablation` ← `replication_B/summary/ablation_summary_*.csv` |
| 8 Per-class | A | `make_tables_rev.t_perclass` (submitted body, unchanged); regenerated from the source by `make_tables_submitted.t_perclass` and checked identical | `scripts/tables_submitted/tab_perclass.tex`; source `replication_A/runs/proposed/*/per_class_*.csv` |
...
| Statistical power, Friedman/Nemenyi, McNemar | §3.8, §4.2 | `replication_A/summary/stats_report.json`, `replication_B/statistics/stats_report.json`, `statistical_power.json`; recomputable with `analyze.py --out_dir paper/replication_A/runs` |
